...
<template>
  <div class=" flex flex-col flex-1 gap-4 p-4 h-full overflow-auto">
    <h2 class="text-3xl font-bold text-custom-blue">{{ assetName }}</h2>
    ...
    />
    <div v-if="transactions && assetName">
      <TransactionsTable :assetName="assetName" :transactions="transactions" />
    </div>
  </div>
</template>
