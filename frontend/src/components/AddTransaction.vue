...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const displayAmount = computed({
      get: () => newTransaction.value.amount === 0 ? '' : newTransaction.value.amount.toString(),
      set: (val) => {
  ...
  <div v-if="showModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-10" @click.self="closeModal">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
      ...
      <form @submit.prevent="submitForm">
        <div class="mt-2">
          <select v-model="newTransaction.payee" class="mt-1 block w-full border-2 border-custom-text rounded " required>
        ...
        <div class="mt-2">
          <select v-model="newTransaction.category" class="mt-1 block w-full border-2 border-custom-text rounded" required>
        ...
        <div class="mt-2">
          <select v-model="newTransaction.type" class="mt-1 block w-full border-2 border-custom-text rounded" required>
        ...
        <div class="mt-2">
          <input v-model.number="displayAmount" type="number" placeholder="Enter an amount" class="mt-1 block w-full border-2 border-custom-text rounded px-1" required>
        </div>
        <div class="mt-2">
          <input v-model="newTransaction.date" type="date" class="mt-1 block w-ful border-2 border-custom-text rounded p-2" required>
        </div>
        <div class="mt-2">
          <select v-model="newTransaction.bank_name" class="mt-1 block w-full border-2 border-custom-text rounded" required>
...
import type { Transaction } from "../domain/transaction";
import type { Account } from "../domain/account";
import { addTransaction } from "@/http/api"; // Update this path to your API call
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const newTransaction = ref<Transaction>({
      id: "",
      account_id: "",
      date: new Date(), // format to YYYY-MM-DD
      bank_name: "",
      payee: "",
      category: "",
      amount: 0,
    ...
    const submitForm = async () => {
      ...
      try {
        const response = await addTransaction(newTransaction.value); // Ensure this function is implemented
        console.log(response);
        emit("add-transaction", response); // Consider emitting the response if it contains the new transaction
        closeModal();
      } catch (error) {
